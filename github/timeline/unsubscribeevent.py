...
if TYPE_CHECKING:
    ...
    class UnsubscribeEventData(NodeData, TimelineItemData, TypeData):
        __typename: Literal["UnsubscribedEvent"]

        subscribable: IssueData | PullData


class UnsubscribeEvent(Node, TimelineItem, Type):
    """
    Represents a unsubscribe event on an :class:`~github.Issue` or
    ...
    _data: UnsubscribeEventData

    _graphql_type = "UnsubscribedEvent"
    ...
    async def fetch_subject(
        ...
        **kwargs,  # TODO
        ...
        data = await self._http.fetch_unsubscribeevent_subject(self.id, **kwargs)
        ...
        if data["__typename"] == "Issue":
            ...
            raise RuntimeError(f"unsupported type {data['__typename']} for UnsubscribedEvent.subscribable")
