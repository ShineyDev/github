...
class HTTPClient(graphql.client.http.HTTPClient):
    ...
    async def fetch_repositorynode_repository(
        ...
        *,
        ...
        fields = github.utility.get_merged_graphql_fields(github.Repository, fields)
        query = "query($repositorynode_id:ID!){node(id:$repositorynode_id){...on Label{repository{%(f)s}}...on Milestone{repository{%(f)s}}...on Ref{repository{%(f)s}}...on RepositoryNode{repository{%(f)s}}}}" % {"f": ",".join(fields)}
