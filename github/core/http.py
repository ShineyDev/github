...
class HTTPClient(graphql.client.http.HTTPClient):
    ...
    async def fetch_repositorynode_repository(
        ...
        *,
        ...
        fields = github.utility.get_merged_graphql_fields(github.Repository, fields)
        query = "query($repositorynode_id:ID!){node(id:$repositorynode_id){...on Label{repository{%(f)s}}...on Milestone{repository{%(f)s}}...on Package{repository{%(f)s}}...on Ref{repository{%(f)s}}...on Release{repository{%(f)s}}...on RepositoryNode{repository{%(f)s}}}}" % {"f": ",".join(fields)}
    ...
    async def mutate_discussion_close(
        self,
        /,
        discussion_id: str,
        ...
        *,
        ...
        fields = ("__typename",) if fields is MISSING else fields
        query = "mutation($closed_reason:DiscussionCloseReason,$discussion_id:ID!,$mutation_id:String!){closeDiscussion(input:{clientMutationId:$mutation_id,discussionId:$discussion_id,reason:$closed_reason}){discussion{%s}}}" % ",".join(fields)
        path = ("closeDiscussion", "discussion")

        data = await self._mutate(query, *path, discussion_id=discussion_id, closed_reason=closed_reason)
    ...
    async def mutate_issue_delete(
        ...
        query = "mutation($issue_id:ID!,$mutation_id:String!){deleteIssue(input:{clientMutationId:$mutation_id,issueId:$issue_id}){__typename}}"

        await self._mutate(query, issue_id=issue_id)

    async def mutate_issue_reopen(
        self,
        /,
        issue_id: str,
        *,
        ...
        fields = ("__typename",) if fields is MISSING else fields
        query = "mutation($issue_id:ID!,$mutation_id:String!){reopenIssue(input:{clientMutationId:$mutation_id,issueId:$issue_id}){issue{%s}}}" % ",".join(fields)
        path = ("reopenIssue", "issue")

        data = await self._mutate(query, *path, issue_id=issue_id)
